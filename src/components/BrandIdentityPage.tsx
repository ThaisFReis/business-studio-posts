    ...
    <div className="flex-1 p-8 overflow-y-auto">
      <div className="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[#0e100f] border border-white/10 rounded-2xl p-6">
          ...
          <div className="rounded-xl border border-white/10 bg-black/40 p-6 min-h-[420px] flex flex-col items-center justify-center text-center">
            {brandIdentity.logoUrl ? (
              <img src={brandIdentity.logoUrl} alt="Brand logo" className="w-24 h-24 object-contain mb-4" />
            ) : (
            ...
            <h4 className="text-2xl font-serif mb-1">{brandIdentity.fullName || 'Brand Company'}</h4>
            <p className="text-xs text-neutral-400 mb-5">{brandIdentity.shortNameWithDot || 'brand.'}</p>
            <p className="text-sm leading-relaxed max-w-[280px] text-neutral-300">{brandIdentity.slogan || 'Your slogan will appear here.'}</p>
          </div>
        </div>
      </div>
    </div>
